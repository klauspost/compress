...
func (s *sequenceDecs) decodeSyncSimple(hist []byte) (bool, error) {
	...
	var errCode int
	if useSafe {
		errCode = sequenceDecs_decodeSync_safe_arm64(s, br, &ctx)
	} else {
		errCode = sequenceDecs_decodeSync_arm64(s, br, &ctx)
	}
	switch errCode {
	case noError:
		break
...
func (s *sequenceDecs) decode(seqs []seqVals) error {
	...
	var errCode int
	if lte56bits {
		errCode = sequenceDecs_decode_56_arm64(s, br, &ctx)
	} else {
		errCode = sequenceDecs_decode_arm64(s, br, &ctx)
	}
	if errCode != 0 {
		...
		return fmt.Errorf("sequenceDecs_decode_arm64 returned erroneous code %d", errCode)
	}
...
func (s *sequenceDecs) executeSimple(seqs []seqVals, hist []byte) error {
	...
	if cap(s.literals) < len(s.literals)+compressedBlockOverAlloc {
		ok = sequenceDecs_executeSimple_safe_arm64(&ctx)
	} else {
		ok = sequenceDecs_executeSimple_arm64(&ctx)
	}
	if !ok {
