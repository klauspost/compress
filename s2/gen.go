...
func genEncodeBlockAsm(name string, tableBits, skipLog int, avx bool) {
		...
		if false {
			...
			JNE(LabelRef("no_repeat_found_" + name))
			{
				...
				// Extend forward
				{
					...
					// candidate := s - repeatL + 4 + checkRep
					MOVL(repeatL, candidate)
					SUBL(s, candidate)
					{
						// srcLeft = sLimitL - s
						srcLeft := GP64()
						MOVL(s, srcLeft.As32())
						SUBL(sLimitL, srcLeft.As32())
						// Forward address
	...
	Label("match_nolit_loop_" + name)
	{
		...
		{
			repeatVal := GP64()
			MOVL(repeatL, repeatVal.As32())
			SUBL(s, repeatVal.As32())
			MOVL(repeatVal.As32(), repeatL)
		}
		// s+=4, candidate+=4
		ADDL(U8(4), s)
		ADDL(U8(4), candidate)
